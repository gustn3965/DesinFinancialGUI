...
######  지수 별 리스트 업데이트.
    ...
    def UpdateIndexList(self, filePath) :
        ...
        self.world1 = {'code': [], 'name': []}
        va = object.GetUsCodeList(2)

        for i in va:
            self.world1['code'].append(i)
            self.world1['name'].append(object.GetNameByUsCode(i))



        self.world2 = {'code': [], 'name': []}
        va = object.GetUsCodeList(3)

        for i in va:
            self.world2['code'].append(i)
            self.world2['name'].append(object.GetNameByUsCode(i))

        worldList1 = pd.DataFrame(self.world1)
        worldList1_name = "LIST_WORLD1_" + nowDate + ".csv"
        worldList1.to_csv(filePath + "/WORLD1/" + worldList1_name, mode='a', index=False,
                         encoding="euc-kr")

        worldList2 = pd.DataFrame(self.world2)
        ...
        etf = {'code': [], 'name': [], 'sectionCode': []}
        etfOverSeas = {'code': [], 'name': []}
        allList = []
        ...
        for i in range(0, 6):
            ...
            print("all code len : " , len(self.codeList))
            for code in self.codeList:
                ...
                if secondCode == 10  :
                    ...
                    etf['sectionCode'].append(secondCode)
                elif secondCode ==  12 :
                    print("12")
                    print("-----"*30)
                    name = instCpCode.CodeToName(code)
                    etfOverSeas['code'].append(code)
                    etfOverSeas['name'].append(name)

                    # print(name, " " , code )

        print("증권전산업 길이 : " , len(instCpCode.GetIndustryList()))
        print("etf 길이 : ", len(etf))
        etfList = pd.DataFrame(etf)
        print(etfList)
        etfOverSeasList = pd.DataFrame(etfOverSeas)
        etf_name = "LIST_ETF_" + nowDate + ".csv"
        etfList.to_csv(filePath + "/ETF/" + etf_name , mode='a', index=False,
                                          encoding="euc-kr")
        etfOverSeasList.to_csv(filePath + "/ETF/etfOverseas.csv", mode='a', index=False,
                       encoding="euc-kr")
...
##            노터치.
    ...
    # 세계 과거 지수
    # ord () !
    def getWorldData(self,world, count, DWM):
        ...
        object.SetInputValue(1, ord(DWM))
        ...
        while count > rcv_count :
            object.BlockRequest()
            rqStatus = object.GetDibStatus()
            if rqStatus != 0:
                return False
            Time.sleep(0.25)
            self.numData = object.GetHeaderValue(3)

            print("종목 코드 : " , object.GetHeaderValue(0))
...
# 8311 과거 세계지수
    def getWorldData8300(self, world, count):
        object = win32com.client.Dispatch("Dscbo1.CpSvr8300")
        object.SetInputValue(0, world)
        ...
        print()
        print()
        print("받은 개수 : ", object.GetHeaderValue(3))
        print(object.GetHeaderValue(0))
        print(object.GetHeaderValue(2))
